...
function switchTab(tabName) {
  ...
  if (tabName === 'dashboard') {
    ...
    if (typeof openChoresTab === 'function') openChoresTab();
  }
}
...
function initChoresDayScroller() {
  ...
  // Gerar semana centrada no dia atual
...
function renderChoresTab(dayIndex) {
  ...
  luanList.innerHTML = data.luan.map((task, i) => buildChoreItem(task, i, 'luan', dayIndex, saved)).join('');
  biancaList.innerHTML = data.bianca.map((task, i) => buildChoreItem(task, i, 'bianca', dayIndex, saved)).join('');
...
// Click on chore items (check/uncheck + edit actions)
