...
const gradlew   = path.join(mobileDir, 'android', 'gradlew.bat');
...
const STEPS = [
  {
    ...
    cmd: 'node', args: ['infra/scripts/copy-web.js'], cwd: root,
  },
  {
    label: '2/4  Capacitor sync',
    cmd: 'npx', args: ['cap', 'sync', 'android'], cwd: mobileDir,
  },
  {
    label: '3/4  Build APK (Gradle)',
    cmd: gradlew, args: ['assembleDebug', '--daemon', '--quiet'], cwd: path.join(mobileDir, 'android'),
  },
  {
    label: '4/4  Upload APK + atualiza URL',
    cmd: 'node', args: ['infra/scripts/upload-apk.js'], cwd: root,
  },
];
